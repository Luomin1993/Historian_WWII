...
re_OBJECT_NAME = re.compile(r'ObjectTemplate.create HandFireArms (.*)');
...
class HandWeapon(object):
    """手持武器实体类"""
    def __init__(self, NameID):
        super(HandWeapon, self).__init__()
        self.NameID = NameID;
        self.MOD_PATH = "";
        self.SM_FILEs = [];
        self.SKE_FILEs = [];
...
def get_obj_weapon(CON_FILE):
    FILE_GEOS = open(CON_FILE, "r"); 
    ALL_LINEs = FILE_GEOS.readlines();
    for STR_LINE in ALL_LINEs:
        if re_OBJECT_NAME.findall(STR_LINE)!=[]:
            OBJ_WEAPON = HandWeapon( re_OBJECT_NAME.findall(STR_LINE)[0] );
            break;
    for STR_LINE in ALL_LINEs:
        OBJ_WEAPON.GEOMETRYs_USED += re_GEOMETRY.findall(STR_LINE);
        OBJ_WEAPON.PROJECTILEs += re_PROJECT.findall(STR_LINE);
        OBJ_WEAPON.EFFECTs += re_EFFECT.findall(STR_LINE);
        OBJ_WEAPON.CREATEs += re_CREAT.findall(STR_LINE);
        OBJ_WEAPON.SKE_FILEs += re_SKESKN.findall(STR_LINE);
        OBJ_WEAPON.TEMPLATEs += re_USED.findall(STR_LINE);
    OBJ_WEAPON.EFFECTs = ["e_"+EFFECT for EFFECT in OBJ_WEAPON.EFFECTs];    
    # OBJ_WEAPON.show_details();
    return OBJ_WEAPON;

# -------- 单个geometries.con文件的资源读取 --------
def process_geometry_file(OBJ_WEAPON,GEO_FILE):
    FILE_GEOS = open(GEO_FILE, "r"); 
    ALL_LINEs = FILE_GEOS.readlines();
    for STR_LINE in ALL_LINEs:
        OBJ_WEAPON.GEOMETRYs_DEF += re_GEOMETRY_DEF.findall(STR_LINE);
        OBJ_WEAPON.SM_FILEs += re_STDMESH.findall(STR_LINE);
    return OBJ_WEAPON;
...
# --------- 贴图路径收集 ---------------
def process_textures(OBJ_WEAPON):
    for SM_FILE in OBJ_WEAPON.SM_FILEs:
        RS_FILE = OBJ_WEAPON.MOD_PATH + "/standardmesh/" + SM_FILE +".rs";
        if not os.path.exists(RS_FILE): continue; # 应该已含有在BF1942原版里;
        DDS_LIST = get_dds_files(RS_FILE);
        for THIS_DDS in DDS_LIST:
            if THIS_DDS not in OBJ_WEAPON.TEXTURE_FILEs:OBJ_WEAPON.TEXTURE_FILEs.append(THIS_DDS);
    return OBJ_WEAPON;    
        
# ================ 手持武器的资源收集  ====================
# -------- 手持武器资源爬取主函数 --------
def copy_operation(OBJ_WEAPON):
    print(" ============ 资源文件复制操作 ===========");
    # 复制SKE文件;
    for SKE_FILE in OBJ_WEAPON.SKE_FILEs:
        ORIGINATING_PATH = OBJ_WEAPON.MOD_PATH + "/animations";
    ...
    # 复制SM/RS文件;
    for SM_FILE in OBJ_WEAPON.SM_FILEs:
        ORIGINATING_PATH = OBJ_WEAPON.MOD_PATH + "/standardmesh";
    ...
    # 复制DDS文件;
    for DDS_FILE in OBJ_WEAPON.TEXTURE_FILEs:
        ORIGINATING_PATH = OBJ_WEAPON.MOD_PATH + "/texture";
        TARGET_PATH = MY_RESOURCE_PATH + "/texture";
        if not os.path.exists(ORIGINATING_PATH + "/" + DDS_FILE + ".dds"):print(DDS_FILE + ".dds");continue;
        shutil.copyfile(ORIGINATING_PATH + "/" + DDS_FILE + ".dds",TARGET_PATH + "/" + DDS_FILE + ".dds");

# -------- 手持武器资源爬取主函数 --------
def read_the_weapon(WEAPON_OBJ_PATH):
    # 处理 Objects.con:
    OBJ_WEAPON = get_obj_weapon(WEAPON_OBJ_PATH+"Objects.con");
    OBJ_WEAPON.MOD_PATH = THE_MOD_PATH;
    # 处理 Geometries.con:
    if os.path.exists(WEAPON_OBJ_PATH + "Geometries.con"):
        OBJ_WEAPON = process_geometry_file(OBJ_WEAPON,WEAPON_OBJ_PATH + "Geometries.con");
    # 处理贴图;
    OBJ_WEAPON = process_textures(OBJ_WEAPON);
    OBJ_WEAPON.TEXTURE_FILEs = list(set(OBJ_WEAPON.TEXTURE_FILEs));
    OBJ_WEAPON.unrepeated();
    OBJ_WEAPON.show_details();
    copy_operation(OBJ_WEAPON);

# -------- 一次性移植多个武器 --------
def batch_process():
    WEAPONS_LIST = ["Mas38","Mas36"];
    for WEAPON in WEAPONS_LIST:
        read_the_weapon(THE_MOD_PATH + "/objects/Handweapons/" + WEAPON +"/");
