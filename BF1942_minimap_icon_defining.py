...
re_VEHICLE_TYPE = "ObjectTemplate.setVehicleType (.*)";
re_MINIMAP = "ObjectTemplate.setMinimapIcon \"(.*)\"";
re_PCO = "ObjectTemplate.create PlayerControlObject (.*)";

LIST_CON_FILE_PATH = ["D:/BF1942/Rebuild_XWWII/Historian_WWII/Patch/objects",
                      "D:/BF1942/Rebuild_XWWII/Historian_WWII/Patch-WF/objects",
                      "D:/BF1942/cache/objects",
                      "D:/BF1942/Isolate-RFA/objects"];


DICT_VEHICLES_TO_ICON = {'VTFighter':'minimap_icon_plane_16x16',
                    'VTDiveBomber':'minimap_icon_VTDiveBomber',
                    'VTScoutCar':'minimap_icon_VTScoutCar',
                    'VTArtillery':'minimap_icon_VTArtillery',
                    'VTLightTank':'minimap_icon_tank_16x16',
                    'VTApc':'minimap_icon_apc_16x16',
                    'VTLcvp':'minimap_icon_common_16x16',
                    'VTDefGun':'minimap_icon_VTDefGun',
                    'VTHeavyTank':'minimap_icon_VTHeavyTank',
                    'VTTankHunter':'minimap_icon_VTTankHunter',
                    'VTMobileArtillery':'minimap_icon_VTMobileArtillery',
                    'VTStationaryMG':'minimap_icon_VTStationaryMG',
                    'VTBomber':'minimap_icon_VTDiveBomber',
                    'VTAAGun':'minimap_icon_VTAAGun',
                    'AAGun':'minimap_icon_VTAAGun',
                    'VTDestroyer':'minimap_icon_battleship_64x64',
                    'VTCarrier':'minimap_icon_battleship_64x64',
                    'VTSubmarine':'minimap_icon_submarine_32x32',
                    'ItLcvp':'minimap_icon_common_16x16',
                    'VTBattleship':'minimap_icon_battleship_64x64'};
...
# =================  字符串提取功能  =====================
"""
提取文件 CON_FILE 中的 re_EFFECT_STR 模式的字符串;
"""
def single_file_find(CON_FILE):
    print(CON_FILE);
    RES_STRINGs = [];
    ...
    for THIS_LINE in ALL_LINES:
        RES_STRINGs += re.findall(re_VEHICLE_TYPE,THIS_LINE);
    RES_STRINGs = [STRING.replace(" ","") for STRING in RES_STRINGs];        
    return RES_STRINGs;
...
def processing_single_file(CON_FILE):
    RES_STRINGs = single_file_find(CON_FILE);
    if RES_STRINGs == []:return;
    TYPE = RES_STRINGs[0];
    if TYPE not in DICT_VEHICLES_TO_ICON.keys():
        single_file_replace(CON_FILE,re_MINIMAP,"ObjectTemplate.setMinimapIcon \"Minimap/minimap_icon_common_16x16.tga\"");
        return;
    single_file_replace(CON_FILE,re_MINIMAP,"ObjectTemplate.setMinimapIcon \"Minimap/" + DICT_VEHICLES_TO_ICON[TYPE] + ".tga\"");
    return;
...
if __name__ == '__main__':
    ...
    for ROOT_PATH in LIST_CON_FILE_PATH:
        ...
        for CON_FILE in FILE_PATH_LIST:
            processing_single_file(CON_FILE);
    # VEHICLES_TYPES_ALL = [];
    # for TYPE in VEHICLES_TYPES:
    #     if TYPE not in VEHICLES_TYPES_ALL:VEHICLES_TYPES_ALL.append(TYPE);
    # print(VEHICLES_TYPES_ALL);